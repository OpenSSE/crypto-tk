...
TEST(relic, pairing)
{
    ...
    for (size_t i = 0; i < ARITHMETIC_TEST_COUNT; i++) {
        ...
        ASSERT_EQ(group.pair(power(z1, e1), power(z2, e2)),
                  power(p12, e1 * e2));
    }
}
...
TEST(ppke, serialization)
{
    ...
    for (; current_p_count < SERIALIZATION_PUNCT_COUNT; current_p_count++) {
        ...
        std::array<uint8_t, sse::crypto::GmppkePrivateKeyShare::kByteSize>
            share_data;
    ...
    for (size_t i = 0; i < SERIALIZATION_TEST_COUNT; i++) {
        ...
        std::array<uint8_t, sse::crypto::GmmppkeCT<M_type>::kCTByteSize>
            ct_data;
...
TEST(ppke, deterministic_correctness)
{
    ...
    for (auto share : keyshares) {
        if (share.get_tag()
            == sse::crypto::Gmppke::NULLTAG) { // first key share
            ASSERT_THROW(ppke.encrypt<M_type>(pk, 0, share.get_tag()),
                         std::invalid_argument);
        } else {
            auto ct = ppke.encrypt<M_type>(pk, 0, share.get_tag());

            ASSERT_THROW(
                ppke.decrypt(sse::crypto::GmppkePrivateKey(keyshares), ct),
                sse::crypto::PuncturedCiphertext);
        }
    }
}
...
TEST(puncturable, correctness)
{
    ...
    for (auto share : punctured_key) {
        if (sse::crypto::punct::extract_tag(share)
            == sse::crypto::Gmppke::NULLTAG) { // first key share
            ASSERT_THROW(
                encryptor.encrypt(0, sse::crypto::punct::extract_tag(share)),
                std::invalid_argument);
        } else {
            auto ct
                = encryptor.encrypt(0, sse::crypto::punct::extract_tag(share));
