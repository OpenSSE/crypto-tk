...
TdpImpl_mbedTLS::TdpImpl_mbedTLS(const std::string& pk)
{
    ...
    if (mbedtls_rsa_check_pubkey(&rsa_key_) != 0) {
        throw std::runtime_error(
            "Invalid public key generated during the TDP initialization"); /* LCOV_EXCL_LINE
                                                                            */
    }
}
...
TdpImpl_mbedTLS::TdpImpl_mbedTLS(const TdpImpl_mbedTLS& tdp)
{
    ...
    if (mbedtls_rsa_check_pubkey(&rsa_key_) != 0) {
        throw std::runtime_error(
            "Invalid public key generated by the TDP copy constructor"); /* LCOV_EXCL_LINE
                                                                          */
    }
}
...
#warning Potentially insecure RSA implementation
    ...
    if (mbedtls_mpi_write_binary(&y, out.data(), out.size()) != 0) {
        throw std::runtime_error(
            "Error while writing RSA result to the out buffer"); /* LCOV_EXCL_LINE
                                                                  */
    }
