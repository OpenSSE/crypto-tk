...
void Gmppke::keygen(Key<kPRFKeySize>&&      prf_key,
                    GmppkePublicKey&        pk,
                    GmppkePrivateKey&       sk,
                    GmppkeSecretParameters& sp) const
{
...
void Gmppke::keygen(const sse::crypto::Prf<kPPKEPrfOutputSize>& prf,
                    GmppkePublicKey&                            pk,
                    GmppkePrivateKey&                           sk,
                    GmppkeSecretParameters&                     sp) const
{
...
void Gmppke::paramgen(const sse::crypto::Prf<kPPKEPrfOutputSize>& prf,
                      GmppkeSecretParameters&                     sp) const
{
...
void Gmppke::keygenPartial(const ZR&                     alpha,
                           GmppkePublicKey&              pk,
                           GmppkePrivateKey&             sk,
                           const GmppkeSecretParameters& sp) const
{
    ...
    //    assert(pk.g2G1 ==
    //    LagrangeInterpInExponent<G1>(group,0,polynomial_xcordinates,pk.gqofxG1));
    //    assert(pk.g2G2 ==
    //    LagrangeInterpInExponent<G2>(group,0,polynomial_xcordinates,pk.gqofxG2));


    sk.shares.push_back(skgen(sp));
}

void Gmppke::keygenPartial(const sse::crypto::Prf<kPPKEPrfOutputSize>& prf,
                           const ZR&                                   alpha,
                           GmppkePublicKey&                            pk,
                           GmppkePrivateKey&                           sk,
                           const GmppkeSecretParameters&               sp) const
...
GmppkePrivateKeyShare Gmppke::skgen(
...
GmppkePrivateKeyShare Gmppke::sk0Gen(
    const sse::crypto::Prf<kPPKEPrfOutputSize>& prf,
    ...
    size_t                                      d) const
{
    ...
    if (d == 0) {
        ...
        //        const ZR rho_d_1 = group.pseudoRandomZR(prf,
        //        std::string("param_rho_%d",d-1));

        const ZR l_d = group.pseudoRandomZR(prf, ("param_l_" + d_string));

        //        const ZR l_d_1 = (d > 1) ? (group.pseudoRandomZR(prf,
        //        std::string("param_l_%d",d-1))) : (-sp.alpha);
...
GmppkePrivateKeyShare Gmppke::skShareGen(
    const sse::crypto::Prf<kPPKEPrfOutputSize>& prf,
    const GmppkeSecretParameters&               sp,
    size_t                                      d,
...
void Gmppke::puncture(const GmppkePublicKey& pk,
                      GmppkePrivateKey&      sk,
                      const tag_type&        tag) const
{
    if (tag == NULLTAG) {
    ...
    skentry0.sk1 = group.mul(
        skentry0.sk1,
        group.exp(pk.g2G2, group.sub(r0, lambda))); // sk1 * g2g2^{r0- lambda}
    const G2 vofx = vx(pk.gqofxG2, NULLTAG);
    skentry0.sk2
        = group.mul(skentry0.sk2, group.exp(vofx, r0)); // sk2 * V(t0)^r0
    skentry0.sk3
        = group.mul(skentry0.sk3, group.exp(pk.gG2, r0)); // sk3 * g2G2^r0

    skentryn.sk1
        = group.exp(pk.g2G2, group.add(r1, lambda)); // gG2 ^ (r1+lambda)
...
PartialGmmppkeCT Gmppke::blind(const GmppkePublicKey& pk,
                               const ZR&              s,
                               const tag_type&        tag) const
{
    if (tag == NULLTAG) {
...
PartialGmmppkeCT Gmppke::blind(const GmppkeSecretParameters& sp,
                               const relicxx::ZR&            s,
                               const tag_type&               tag) const
{
    if (tag == NULLTAG) {
                        ...
                        const PartialGmmppkeCT& ct) const
{
    ...
    for (unsigned int i = 0; i < numshares; i++) {
        ...
        ZR w0 = LagrangeBasisCoefficients<2>(
            group, 0, zr_zero, {{ctTag, currentTag}});
        const ZR wstar = LagrangeBasisCoefficients<2>(
            group, 1, zr_zero, {{ctTag, currentTag}});
        ...
        GT denominator
            = group.mul(group.pair(ct3prod_j, s0.sk3),
                        group.pair(group.exp(ct.ct2, wstar), s0.sk2));
