...
{
    string in = "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklm"
...
TEST(hash, consistency)
{
    for (size_t i = 1; i < sse::crypto::Hash::kDigestSize; i++) {
        ...
        sse::crypto::Hash::hash(reinterpret_cast<const uint8_t*>(in.data()),
                                in.size(),
                                out_array.data());
        sse::crypto::Hash::hash(reinterpret_cast<const uint8_t*>(in.data()),
                                in.size(),
                                i,
                                trunc_out_array.data());

        ASSERT_EQ(trunc_out, std::string(out.begin(), out.begin() + i));
        ASSERT_EQ(out, std::string(out_array.begin(), out_array.end()));
        ASSERT_EQ(
            trunc_out,
            std::string(trunc_out_array.begin(), trunc_out_array.begin() + i));
    }
}
...
TEST(hash, exceptions)
{
    ...
    ASSERT_THROW(
        sse::crypto::Hash::hash(in, sse::crypto::Hash::kDigestSize + 1),
        std::invalid_argument);

    ASSERT_THROW(sse::crypto::Hash::hash(
                     reinterpret_cast<const uint8_t*>(in.data()), 0, NULL),
                 std::invalid_argument);
    ASSERT_THROW(sse::crypto::Hash::hash(NULL,
                                         0,
    ...
    ASSERT_THROW(
        sse::crypto::Hash::hash(
            ...
            0,
            sse::crypto::Hash::kDigestSize + 1,
            reinterpret_cast<unsigned char*>(const_cast<char*>(out.data()))),
        std::invalid_argument);
    ASSERT_THROW(sse::crypto::Hash::hash(NULL,
                                         0,
                                         sse::crypto::Hash::kDigestSize - 1,
