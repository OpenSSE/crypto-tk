...
TdpInverseImpl_mbedTLS::TdpInverseImpl_mbedTLS()
{
    ...
    {
        throw std::runtime_error("Failed MPI substraction"); /* LCOV_EXCL_LINE */
    }
    ...
    {
        throw std::runtime_error("Failed MPI multiplication"); /* LCOV_EXCL_LINE */
    }
}
...
std::array<uint8_t, TdpInverseImpl_mbedTLS::kMessageSpaceSize> TdpInverseImpl_mbedTLS::invert_mult(const std::array<uint8_t, kMessageSpaceSize> &in, uint32_t order) const
{
...
void TdpInverseImpl_mbedTLS::invert_mult(const std::string &in, std::string &out, uint32_t order) const
{
    std::array<uint8_t, kMessageSpaceSize> in_array;
...
TdpMultPoolImpl_mbedTLS::TdpMultPoolImpl_mbedTLS(const std::string& sk, const uint8_t size)
: TdpImpl_mbedTLS(sk), keys_count_(size-1)
{
    ...
    {
        throw std::runtime_error("Invalid public key generated during the TDP pool initialization"); /* LCOV_EXCL_LINE */
    }
...
TdpMultPoolImpl_mbedTLS::TdpMultPoolImpl_mbedTLS(const TdpMultPoolImpl_mbedTLS& pool_impl)
: TdpImpl_mbedTLS(pool_impl), keys_count_(pool_impl.keys_count_)
{
    ...
    for (uint8_t i = 0; i < keys_count_; i++) {
        ...
        if(mbedtls_rsa_check_pubkey(&keys_[i]) != 0)
        {
            throw std::runtime_error("Invalid public key generated by the TDP pool copy constructor"); /* LCOV_EXCL_LINE */
        }
    }
}

TdpMultPoolImpl_mbedTLS& TdpMultPoolImpl_mbedTLS::operator=(const TdpMultPoolImpl_mbedTLS& t)
{
    if (this != & t) {
        for (uint8_t i = 0; i < keys_count_; i++) {
            zeroize_rsa(&keys_[i]);
        ...
        for (uint8_t i = 0; i < keys_count_; i++) {
            ...
            if(mbedtls_rsa_check_pubkey(&keys_[i]) != 0)
            {
                throw std::runtime_error("Invalid public key generated by the TDP pool assignment operator"); /* LCOV_EXCL_LINE */
            }
        }
    }
    return *this;
...
std::array<uint8_t, TdpImpl_mbedTLS::kMessageSpaceSize> TdpMultPoolImpl_mbedTLS::eval_pool(const std::array<uint8_t, kMessageSpaceSize> &in, const uint8_t order) const
{
    std::array<uint8_t, TdpImpl_mbedTLS::kMessageSpaceSize> out;
...
void TdpMultPoolImpl_mbedTLS::eval_pool(const std::string &in, std::string &out, const uint8_t order) const
{
    if(in.size() != rsa_size())
    {
        throw std::invalid_argument("Invalid TDP input size. Input size should be kMessageSpaceSize bytes long.");
