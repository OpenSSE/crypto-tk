 ...
 *  Reference vectors are taken from RFC 4231
 *[https://tools.ietf.org/html/rfc4231] Only the first four test cases are
 *implemented: the HMAC-based PRF implementation does not support keys larger
 *than 64 bytes
 ********/
...
template<uint16_t N>
using HMAC_SHA512 = sse::crypto::HMac<sse::crypto::hash::sha512, N>;
...
0x79, 0x61, 0x20, 0x77, 0x61, 0x6e, 0x74, 0x20, 0x66, 0x6f, 0x72, 0x20, 0x6e,
...
0xe3, 0x95, 0xfb, 0xe7, 0x3b, 0x56, 0xe0, 0xa3, 0x87, 0xbd, 0x64, 0x22, 0x2e,
0x83, 0x1f, 0xd6, 0x10, 0x27, 0x0c, 0xd7, 0xea, 0x25, 0x05, 0x54, 0x97, 0x58,
0xbf, 0x75, 0xc0, 0x5a, 0x99, 0x4a, 0x6d, 0x03, 0x4f, 0x65, 0xf8, 0xf0, 0xe6,
0xfd, 0xca, 0xea, 0xb1, 0xa3, 0x4d, 0x4a, 0x6b, 0x4b, 0x63, 0x6e, 0x07, 0x0a,
0x38, 0xbc, 0xe7, 0x37
                                    }};
...
TEST(hmac_sha_512, test_vector_4)
{
    ...
    array<uint8_t, 64> result_64
        = hmac.hmac(std::string(reinterpret_cast<char*>(in), 50));
...
TEST(hmac, exception)
{
    ASSERT_THROW(HMAC_SHA512<25> hmac(sse::crypto::Key<25>(NULL)),
    ...
    ASSERT_THROW(hmac.hmac(&c, 1, &c, HMAC_SHA512<25>::kDigestSize + 10),
                 std::invalid_argument);
}
