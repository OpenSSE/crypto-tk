...
static void storeu(void* p, __m128i x)
{
...
static uint8x16_t bswap16(uint8x16_t b)
{
    return b;
} /* Not with uint8x16_t */
...
static uint8x16_t aes4(uint8x16_t in,
                       uint8x16_t a,
                       uint8x16_t b,
                       uint8x16_t c,
                       uint8x16_t d)
...
static uint8x16_t load(const void* p)
{
    return *(uint8x16_t*)p;
}
static void store(void* p, uint8x16_t x)
{
    *(uint8x16_t*)p = x;
}
...
void aez_setup(const unsigned char* key, unsigned keylen, aez_ctx_t* ctx)
{
    ...
    if (keylen == 48) {
        ...
        crypto_generichash_blake2b(
            (unsigned char*)ctx, 48, key, keylen, NULL, 0);
...
static block aez_hash(const aez_ctx_t* ctx,
                      const char*      n,
                      unsigned         nbytes,
                      unsigned         abytes)
{
...
static block pass_one(const aez_ctx_t* ctx,
                      const block*     src,
                      unsigned         bytes,
                      block*           dst)
{
    ...
    while (bytes >= 16 * 16) {
        ...
        store(dst + 10,
              aes4(vxor(load(src + 11), o5), J, I, L, load(src + 10)));
        store(dst + 12,
              aes4(vxor(load(src + 13), o6), J, I, L, load(src + 12)));
        store(dst + 14,
              aes4(vxor(load(src + 15), o7), J, I, L, load(src + 14)));
        tmp = aes4(load(dst + 0), J, I, L, load(src + 1));
        store(dst + 1, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 2), J, I, L, load(src + 3));
        store(dst + 3, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 4), J, I, L, load(src + 5));
        store(dst + 5, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 6), J, I, L, load(src + 7));
        store(dst + 7, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 8), J, I, L, load(src + 9));
        store(dst + 9, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 10), J, I, L, load(src + 11));
        store(dst + 11, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 12), J, I, L, load(src + 13));
        store(dst + 13, tmp);
        sum = vxor(sum, tmp);
        tmp = aes4(load(dst + 14), J, I, L, load(src + 15));
        store(dst + 15, tmp);
        sum = vxor(sum, tmp);
        bytes -= 16 * 16;
        dst += 16;
...
static block pass_two(const aez_ctx_t* ctx, block s, unsigned bytes, block* dst)
{
    block o0;
    block o1;
    block o2;
    ...
    while (bytes >= 16 * 16) {
        ...
        fs[1]  = aes4(vxor(s, o1), L, I, J, L);
        fs[2]  = aes4(vxor(s, o2), L, I, J, L);
        fs[3]  = aes4(vxor(s, o3), L, I, J, L);
        fs[4]  = aes4(vxor(s, o4), L, I, J, L);
        fs[5]  = aes4(vxor(s, o5), L, I, J, L);
        fs[6]  = aes4(vxor(s, o6), L, I, J, L);
        fs[7]  = aes4(vxor(s, o7), L, I, J, L);
        tmp[0] = vxor(load(dst + 0), fs[0]);
        sum    = vxor(sum, tmp[0]);
        store(dst + 0, vxor(load(dst + 1), fs[0]));
        tmp[1] = vxor(load(dst + 2), fs[1]);
        sum    = vxor(sum, tmp[1]);
        store(dst + 2, vxor(load(dst + 3), fs[1]));
        tmp[2] = vxor(load(dst + 4), fs[2]);
        sum    = vxor(sum, tmp[2]);
        store(dst + 4, vxor(load(dst + 5), fs[2]));
        tmp[3] = vxor(load(dst + 6), fs[3]);
        sum    = vxor(sum, tmp[3]);
        store(dst + 6, vxor(load(dst + 7), fs[3]));
        tmp[4] = vxor(load(dst + 8), fs[4]);
        sum    = vxor(sum, tmp[4]);
        store(dst + 8, vxor(load(dst + 9), fs[4]));
        tmp[5] = vxor(load(dst + 10), fs[5]);
        sum    = vxor(sum, tmp[5]);
        store(dst + 10, vxor(load(dst + 11), fs[5]));
        tmp[6] = vxor(load(dst + 12), fs[6]);
        sum    = vxor(sum, tmp[6]);
        store(dst + 12, vxor(load(dst + 13), fs[6]));
        tmp[7] = vxor(load(dst + 14), fs[7]);
        sum    = vxor(sum, tmp[7]);
        ...
        store(dst + 10,
              aes4(vxor(load(dst + 11), o5), J, I, L, load(dst + 10)));
        store(dst + 12,
              aes4(vxor(load(dst + 13), o6), J, I, L, load(dst + 12)));
        store(dst + 14,
              aes4(vxor(load(dst + 15), o7), J, I, L, load(dst + 14)));
        bytes -= 16 * 16;
        dst += 16;
    }
    if (bytes >= 8 * 16) {
...
static int cipher_aez_core(const aez_ctx_t* ctx,
                           block            t,
                           int              d,
                           const char*      src,
                           unsigned         bytes,
...
static int cipher_aez_tiny(const aez_ctx_t* ctx,
                           block            t,
                           int              d,
                           const char*      src,
                           unsigned         bytes,
...
void aez_encrypt(const aez_ctx_t* ctx,
                 const char*      n,
                 unsigned         nbytes,
                 unsigned         abytes,
                 const char*      src,
...
int aez_decrypt(const aez_ctx_t* ctx,
                const char*      n,
                unsigned         nbytes,
                unsigned         abytes,
                const char*      src,
